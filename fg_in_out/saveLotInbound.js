...
const BASE_URL =
  "http://203.154.184.162:5012/api/logProductFgEditAmount/SaveLotInOutBound";
const AUTH_TOKEN =
...
function getDynamicPayload() {
  const uniqueId = Date.now() + "-" + Math.random().toString(36).substring(7); // ป้องกันค่า `id` ซ้ำกัน
  return JSON.stringify({
    id: "f0920638-0a1c-4ef7-929a-df7678054bb8",
    edit_type: true, // สุ่มค่า true/false
    amount: Math.floor(Math.random() * 50) + 1, // ปริมาณสินค้า 1 - 50
    lot_no: "20250204-0001",
    exp_date: new Date(
      new Date().setMonth(new Date().getMonth() + 6)
    ).toISOString(), // Expired อีก 6 เดือน
